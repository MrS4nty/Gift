...
function start() {
	...
	if (gameover) {
		...
		crc.drawImage(img, 142, 1069, 300, 90, Math.ceil(viewSize.width * 0.7 - k * 277 * 0.5), Math.ceil(200 / 800 * viewSize.height), 300 * k, 80 * k);
		...
		if (playing) {
			...
			crc.drawImage(img, 179, 837, 300, 50, Math.ceil((viewSize.width - 210 * k) * 0.5), Math.ceil(260 / 800 * viewSize.height), 380 * k, 90 * k);
		}
		score.draw();
		ground.draw();
